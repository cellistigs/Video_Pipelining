...
def SelectFramesFinetune(videopath,video,clip,x1,x2,y1,y2,cropping,Task,indices):
    ...
    indexlength = int(np.ceil(np.log10(clip.duration * clip.fps)))
    for index in np.sort(frames2pick):
        print(index)
        try:
...
if __name__ == '__main__':
    ...
    y2 = 480
    Task = 'social_v4'
    video = 'V118_03182018_cohousing-Camera-1-Animal-1-Segment1.avi'
    videopath = "../DeepLabCut/videos/S3/V118/V118_03182018/"
    all_indices = [36000*9+np.array([12163,12164,17289,17290,17300,17311,17312,17340,17347,17372,
                    17389,19146,19148,19161,19808,19820,31101,24960,25770,32846,
                    32922,32930,33118,32040,32045,32760,32850]),36000*19+np.array([13183,13184,13192,
                    ...
                    15890,16170,16171,23850,23851,30180,30181]),36000*29+np.array([1966,1967,3292,1530,
    ...
    for i,indices in enumerate(all_indices):
        ...
        SelectFramesFinetune(videopath,video,clip,x1,x2,y1,y2,cropping,Task,indices)
